class Solution {
    public int findMinDifference(List<String> timePoints) {
        ...
        int prevIndex = Integer.MAX_VALUE, firstIndex = Integer.MAX_VALUE, lastIndex = Integer.MAX_VALUE, ans = Integer.MAX_VALUE;
        for (int i = 0; i < 24 * 60; i++) {
            if (minutes[i]) {
                if (prevIndex != Integer.MAX_VALUE) ans = Math.min(ans, i - prevIndex);
                prevIndex = i;
